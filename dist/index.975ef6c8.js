...
parcelHelpers.export(exports, "cardsURL", ()=>cardsURL);
/* Считаем цены со скидкой */ parcelHelpers.export(exports, "calcPrice", ()=>calcPrice);
parcelHelpers.export(exports, "basketArray", ()=>basketArray);
parcelHelpers.export(exports, "fullPrice", ()=>fullPrice);
...
const cardsURL = "https://63fb14da2027a45d8d5fb8bf.mockapi.io/cards";
(0, _searchJs.renderSearchCards)() // search cards
;
...
/* Cards - Sales Hits */ async function salesHits() {
    const response = await fetch(cardsURL);
    const cardsArr = await response.json();
    /* Creating cards  */ for(let i = 0; i < 10; i++){
        let itemHTML = `
    ...
    /* Creating POP-UP */ const buttons = document.querySelectorAll(".card__view");
    ...
    /* Closing Pop-Up */ const closePopUp = document.querySelector(".pop-up__content__close");
    ...
    /* Buttons to the Cart */ buttonsAdd.forEach((button, i)=>{
        button.addEventListener("click", ()=>{
            /* LocalStorage */ basketArray.push(cardsArr[i]);
            fullPrice += calcPrice(cardsArr[i].price, cardsArr[i].sale);
            localStorage.setItem("price", JSON.stringify(fullPrice));
            localStorage.setItem("basket", JSON.stringify(basketArray));
            /* Animation for button after adding a product */ button.classList.toggle("card__item-add--click"), setTimeout(function() {
                button.classList.remove("card__item-add--click");
            }, 800);
        });
    });
    /* Same buttons but popup */ popupAdd.forEach((button, i)=>{
        button.addEventListener("click", ()=>{
...
async function setLocalStorage(idCards) {
    const response = await fetch(cardsURL);
    const cardsArr = await response.json();
    for(let i = 0; i < cardsArr.length; i++)if (cardsArr[i].idElem === idCards) {
...
salesHits();
/* Counting items in the cart */ const countItems = document.getElementById("count__items");
function checkBasket(arr) {
    if (arr.length > 0) {
        countItems.style.display = "flex";
        countItems.innerHTML = arr.length;
    } else countItems.style.display = "none";
}
...
},{"./search.js":"8gVq6","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["igKGj","8lqZg"], "8lqZg", "parcelRequire716c")
